...
BitVector *bv_create(uint32_t length) {
    ...
    // allocates memory for bitvector v, initalizes length, and allocates memory for v->vector
    v->length = length;
...
uint8_t bv_get_bit(BitVector *v, uint32_t i) {
    // returns a bit from a specified location that is shifted it to the Least value bit
